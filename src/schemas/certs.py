...
class Cert(BaseModel):
    ...
    begin_time: str = Field(..., example="1970-01-01 00:00:00", description="Begin SSLCert time")
    expire_time: str = Field(..., example="2038-01-19 03:14:07", description="Expire SSLCert time")
    begin_unixtime: int  =  Field(..., example=0, description="Begin SSLCert unixtime")
    expire_unixtime: int = Field(..., example=2147483647, description="Expire SSLCert unixtime")
    issuer: str = Field(..., example="Let's Encript", description="SSLCert Issuer")
...
class CertExpiryCheckResponse(BaseModel):
    id: int
    result: bool = Field(..., example="True", description="Cert Check Result")
    domain: str = Field(..., example="www.example.com", description="Domain Name")
    port: int = Field(..., example=443, description="Port Number")
    message: Optional[str] = Field(..., example="[Errno 2] No such file or directory: 'not_exist_file'", description="Reason for Exception")


class AllCertExpiryCheckResponse(BaseModel):
    cert_size: int = Field(..., example="777", description="Checked number of cert")
